...
arr2 = []
arr3 = []
3000.times { |i| arr1 << a.split[i] if i % 3 == 0} 
3000.times { |i| arr2 << a.split[i] if i % 3 == 1} 
3000.times { |i| arr3 << a.split[i] if i % 3 == 2}
valid = [] 
1000.times do |i|
  position1 = arr1[i].slice(/\d+-/).slice(/\d+/).to_i - 1
  position2 = arr1[i].slice(/-\d+/).slice(/\d+/).to_i - 1
  source = arr3[i]
  target = arr2[i].slice(/[a-z]/)
  validate = (source[position1] == target && source[position2] != target || source[position1] != target && source[position2] == target)
  valid << 1 if validate
end
p valid.length