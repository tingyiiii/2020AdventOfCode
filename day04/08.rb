...
iyr:2010 byr:1950 pid:405416908".split("\n"+"\n").map!{ |i| i.split }
arr.map{ |i| i.map!{ |i| i.split(':') }.map{|arr| arr[0] = arr[0].to_sym } }
arr_hash = arr.map{ |i| i.to_h }
answer = []
arr_hash.map do |i|
  answer << i if i.has_key?(:byr) && i.has_key?(:iyr) && i.has_key?(:eyr) && i.has_key?(:hgt) && i.has_key?(:hcl) && i.has_key?(:ecl) && i.has_key?(:pid)
end

validate = []
def hgt_validate(height)
  if height.include?("cm")
    height.slice(/\d*/).to_i.between?(150,193)
  elsif height.include?("in")
    height.slice(/\d*/).to_i.between?(59,76)
  else
    false
  end
end

def hcl_validate(color)
  color.length == 7 && color.match?(/#([a-f0-9]{6})/)
end

def ecl_validate(color)
  case color
...
def pid_validate(pid)
  pid.length == 9 && pid.match?(/[0-9]{6}/)
end

answer.map do |i|
  validate << i if i[:byr].to_i.between?(1920,2002) && i[:iyr].to_i.between?(2010,2020) && i[:eyr].to_i.between?(2020,2030) && hgt_validate(i[:hgt]) && hcl_validate(i[:hcl]) && ecl_validate(i[:ecl]) && pid_validate(i[:pid])
end
p validate.length